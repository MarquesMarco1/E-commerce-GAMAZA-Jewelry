...
export default function Content() {
    ...
    const stateManager = ['Confirm Address', 'Select Shipping Method', 'Go to Payment'];
    ...
    useEffect(() => {
        const fetchUserData = async () => {
            ...
            if (user) {
                const validateAdress = await fetch(`${localhost}/api/validateAdress/${user}`);
                ...
                if (Object.keys(data.isAdressValide).length > 0) {
                    countryList.blacklist.map((country) => {
                        if (country.countryCode !== data.isAdressValide.country) {
                            ...
                            alert("Can't deliver in this country. You can change it in your profile")
                            navigate('/profile')
                        }
                    })
                }
        ...
        cart.forEach((item) => {
            total +=
                ...
                (
                    item.product.price *
                    item.itemQty *
                    ((item.product.promotion.id !== 1 ? item.product.promotion.pourcentage : 0) /
                        100)
                ).toFixed();
        });
        setSubTotal(total);
